...
class Detections:
    ...
    def _run(self, pprint=False, show=False, save=False, crop=False, render=False, labels=True, save_dir=Path(""), threshold=0.8):
            ...
            if pred.shape[0]:
                ...
                if show or save or render or crop:
                    ...
                    for *box, conf, cls in reversed(pred):  # xyxy, confidence, class
                        ...
                        if conf < threshold:
                            LOGGER.info(f"Skipping detection as confidence is below threshold: {conf} < {threshold}")
                            file = save_dir / "crops" / "below-threshold" / self.names[int(cls)] / self.files[i] if save else None
